package packer_test

import (
	"log"
	"reflect"
	"testing"
	"time"

	packer "github.com/paveldroo/go-ogg-packer"
	"github.com/paveldroo/go-ogg-packer/opus"
	"github.com/paveldroo/go-ogg-packer/tests/testutil"
)

const headersCount = 39

func TestPacker(t *testing.T) {
	tests := []struct {
		name        string
		sampleRate  int
		sourceFname string
		refFname    string
		wantErr     bool
		errByte     int16
	}{
		{
			name:        "8k 1ch",
			sampleRate:  8000,
			sourceFname: "tests/testdata/8k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/8k_1ch.pcm",
		},
		{
			name:        "12k 1ch",
			sampleRate:  12000,
			sourceFname: "tests/testdata/12k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/12k_1ch.pcm",
		},
		{
			name:        "16k 1ch",
			sampleRate:  16000,
			sourceFname: "tests/testdata/16k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/16k_1ch.pcm",
		},
		{
			name:        "24k 1ch",
			sampleRate:  24000,
			sourceFname: "tests/testdata/24k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/24k_1ch.pcm",
		},
		{
			name:        "48k 1ch",
			sampleRate:  48000,
			sourceFname: "tests/testdata/48k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/48k_1ch.pcm",
		},
		{
			name:        "48k 1ch want error",
			sampleRate:  48000,
			sourceFname: "tests/testdata/48k_1ch.pcm",
			refFname:    "tests/testdata/want/packer/48k_1ch.pcm",
			wantErr:     true,
			errByte:     1,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sourcePCMData := testutil.PCMData(t, tt.sourceFname)
			cfg := opus.Config{
				SampleRate:  tt.sampleRate,
				NumChannels: opus.NumChannels,
				FrameSize:   time.Duration(opus.FrameSize) * time.Millisecond,
			}

			p, err := packer.New(cfg)
			if err != nil {
				t.Fatalf("create new packer: %s", err.Error())
			}

			for i := 0; i < len(sourcePCMData); i++ {
				end := min(i+2048, len(sourcePCMData))
				if err := p.SendPCMChunk(sourcePCMData[i:end]); err != nil {
					t.Fatalf("send PCM chunk: %s", err.Error())
				}
				i = end
			}

			audioData, err := p.GetResult()
			if err != nil {
				log.Fatalf("get result from packer: %s", err.Error())
			}

			pcm := testutil.PCMFromOgg(t, audioData, tt.sampleRate, opus.NumChannels)

			refData := testutil.PCMData(t, tt.refFname)

			if tt.wantErr {
				pcm = append(pcm, tt.errByte)
				if reflect.DeepEqual(refData, pcm) {
					t.Fatal("source data and want data should NOT be equal")
				}
				return
			}

			if mse := testutil.CalculateMSE(t, refData, pcm); mse > 5.0 {
				t.Fatalf("significant distortions in reference and result files, mse: %f", mse)
			}
		})
	}
}
